...
def main_panel(request):
    drivers_stats = [(Action.objects.filter(driver=x.user).last(),i) for x,i in zip(Profile.objects.filter(is_driver=True), range(1,1+len(Profile.objects.filter(is_driver=True))))]
    print(drivers_stats)
